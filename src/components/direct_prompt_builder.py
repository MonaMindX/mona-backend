"""
Prompt builder for direct query processing for the non-RAG requests.
"""

from haystack.components.builders import PromptBuilder


def initialize_direct_prompt_builder() -> PromptBuilder:
    """
    Initialize the DirectPromptBuilder component.

    Returns:
        DirectPromptBuilder instance with the direct query prompt.
    """
    template = """You are Mona, a friendly and helpful AI assistant. Your purpose is to engage in conversation, answer general knowledge questions, and assist with tasks that do not require accessing specific documents.
...
User Query: {{ query }}
Assistant Answer:"""
    return PromptBuilder(template=template, required_variables=["query"])
